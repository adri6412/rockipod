...
static int browser(void *param) {
  ...
  case GO_TO_FILEBROWSER:
    ...
    if (global_settings.browse_current && last_screen == GO_TO_WPS &&
  ...
  case GO_TO_DBBROWSER:
    if (!tagcache_is_usable()) {
      ...
      while (!tagcache_is_usable()) {
        ...
        /* Re-init if required */
        if (!reinit_attempted && !stat->ready && stat->processed_entries == 0 &&
            stat->commit_step == 0) {
          /* Prompt the user */
        ...
        if (stat->commit_step > 0) {
          ...
          if (lang_is_rtl()) {
            splash_progress(stat->commit_step, tagcache_get_max_commit_step(),
  ...
  if (ret_val == GO_TO_WPS || ret_val == GO_TO_PREVIOUS_MUSIC ||
...
static int wpsscrn(void *param) {
  ...
  if (audstatus) {
    ...
    if (playlist_resume() != -1) {
      playlist_resume_track(
          global_status.resume_index, global_status.resume_crc32,
  ...
  if (ret_val == GO_TO_PLAYLIST_VIEWER || ret_val == GO_TO_PLUGIN ||
      ret_val == GO_TO_WPS || ret_val == GO_TO_PREVIOUS_MUSIC ||
...
extern struct menu_item_ex file_menu,
#ifdef HAVE_TAGCACHE
    tagcache_menu,
#endif
    main_menu_, manage_settings, plugin_menu, playlist_options, info_menu,
    system_menu;
static const struct root_items items[] = {
    [GO_TO_FILEBROWSER] = {browser, (void *)GO_TO_FILEBROWSER, &file_menu},
#ifdef HAVE_TAGCACHE
    [GO_TO_DBBROWSER] = {custom_db_browser_main, NULL, &tagcache_menu},
#endif
    ...
    [GO_TO_PLAYLISTS_SCREEN] = {playlist_view_catalog, NULL, &playlist_options},
...
MENUITEM_RETURNVALUE_DYNTEXT(wps_item, GO_TO_WPS, NULL, get_wps_item_name, NULL,
                             NULL, Icon_Playback_menu);
#ifdef HAVE_RECORDING
MENUITEM_RETURNVALUE(rec, ID2P(LANG_RECORDING), GO_TO_RECSCREEN, NULL,
                     Icon_Recording);
#endif
#if CONFIG_TUNER
MENUITEM_RETURNVALUE(fm, ID2P(LANG_FM_RADIO), GO_TO_FM, item_callback,
                     Icon_Radio_screen);
#endif
MENUITEM_RETURNVALUE(menu_, ID2P(LANG_SETTINGS), GO_TO_MAINMENU, NULL,
                     Icon_Submenu_Entered);
MENUITEM_RETURNVALUE(bookmarks, ID2P(LANG_BOOKMARK_MENU_RECENT_BOOKMARKS),
                     GO_TO_RECENTBMARKS, item_callback, Icon_Bookmark);
MENUITEM_RETURNVALUE(playlists, ID2P(LANG_PLAYLISTS), GO_TO_PLAYLISTS_SCREEN,
                     NULL, Icon_Playlist);
MENUITEM_RETURNVALUE(system_menu_, ID2P(LANG_SYSTEM), GO_TO_SYSTEM_SCREEN, NULL,
                     Icon_System_menu);

#include "coverflow_app.h"

static int run_coverflow(void *param) {
  (void)param;
  if (coverflow_app())
    return GO_TO_WPS;
  return GO_TO_ROOT;
...
static char *get_coverflow_name(int selected_item, void *data, char *buffer,
                                size_t buffer_len) {
  (void)selected_item;
  (void)data;
  (void)buffer;
...
MENUITEM_FUNCTION_DYNTEXT(coverflow_item, 0, run_coverflow, get_coverflow_name,
                          NULL, NULL, NULL, Icon_Plugin);
  ...
  while (next && menu_item_count < MAX_MENU_ITEMS) {
    ...
    for (i = 0; i < MAX_MENU_ITEMS; i++) {
      if (*start && !strcmp(start, menu_table[i].string)) {
        root_menu__[menu_item_count++] =
            (struct menu_item_ex *)menu_table[i].item;
...
static inline int load_screen(int screen) {
  ...
  if (activity != ACTIVITY_UNKNOWN) {
    if (ret_val == GO_TO_PLUGIN || ret_val == GO_TO_WPS ||
        ret_val == GO_TO_PREVIOUS_MUSIC || ret_val == GO_TO_PREVIOUS_BROWSER ||
...
static int root_menu_setup_screens(void) {
  ...
  if (new_screen == GO_TO_PLUGIN) {
    ...
    if (global_status.last_screen == GO_TO_SHORTCUTMENU ||
        global_status.last_screen == GO_TO_PLUGIN) {
      if (global_settings.start_in_screen ==
          0) { /* Start in: Previous Screen */
  ...
  while (true) {
    ...
    case GO_TO_PLUGIN: {
      ...
      if (next_screen == GO_TO_PREVIOUS) {
        ...
        if (tree_get_context()->out_of_tree >
            0) /* a shortcut has been selected */
        {
      ...
      previous_browser =
          (next_screen != GO_TO_WPS) ? browser_default() : GO_TO_PLUGIN;
      break;
    }
    default:
      goto load_next_screen;
