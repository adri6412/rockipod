...
int main(void) {
  ...
  if (!file_exists(ROCKBOX_DIR "/playername.txt")) {
    int fd =
        open(ROCKBOX_DIR "/playername.txt", O_CREAT | O_WRONLY | O_TRUNC, 0666);
    if (fd >= 0) {
...
static void init(void) {
  ...
  if (!mounted) {
    ...
    if (rc <= 0) {
      ...
      lcd_putsf(0, line++, "id: '%s' s:%u*%u", sinfo.product, sinfo.sector_size,
                sinfo.phys_sector_mult);
#else
      ...
      for (int i = 0; i < NUM_VOLUMES; i++) {
        ...
        if (pinfo.type)
          lcd_putsf(0, line++, "P%d T%02x S%llx", i, pinfo.type,
