...
static int selectivesoftlock_set_mask(void *param) {
  ...
  mask = mask_select(mask, ID2P(LANG_SOFTLOCK_SELECTIVE), maskitems,
                     ARRAYLEN(maskitems));
...
#endif
MENUITEM_SETTING(tagcache_autoupdate, &global_settings.tagcache_autoupdate,
                 NULL);
MENUITEM_FUNCTION(tc_init, 0, ID2P(LANG_TAGCACHE_FORCE_UPDATE),
                  (int (*)(void))tagcache_rebuild_with_splash, NULL,
                  Icon_NOICON);
MENUITEM_FUNCTION(tc_update, 0, ID2P(LANG_TAGCACHE_UPDATE),
                  (int (*)(void))tagcache_update_with_splash, NULL,
                  Icon_NOICON);
MENUITEM_SETTING(runtimedb, &global_settings.runtimedb, NULL);

MENUITEM_FUNCTION(tc_export, 0, ID2P(LANG_TAGCACHE_EXPORT), tagtree_export,
                  NULL, Icon_NOICON);

MENUITEM_FUNCTION(tc_import, 0, ID2P(LANG_TAGCACHE_IMPORT), tagtree_import,
                  NULL, Icon_NOICON);
MENUITEM_FUNCTION(tc_paths, 0, ID2P(LANG_SELECT_DATABASE_DIRS), dirs_to_scan,
                  NULL, Icon_NOICON);
...
MAKE_MENU(tagcache_menu, ID2P(LANG_TAGCACHE), 0, Icon_NOICON,
          ...
          &tagcache_ram,
#endif
          &tagcache_autoupdate, &tc_init, &tc_update, &runtimedb, &tc_export,
          &tc_import, &tc_paths);
#endif /* HAVE_TAGCACHE */
/*    TAGCACHE MENU                */
/***********************************/
...
static int clear_start_directory(void) {
  path_append(global_settings.start_directory, PATH_ROOTSTR, PA_SEP_HARD,
...
static int filemenu_callback(int action, const struct menu_item_ex *this_item,
                             struct gui_synclist *this_list);
MAKE_MENU(file_menu, ID2P(LANG_FILE), filemenu_callback, Icon_file_view_menu,
          &sort_case, &sort_dir, &sort_file, &interpret_numbers, &dirfilter,
          &show_filename_ext, &browse_current, &show_path_in_browser,
...
static int filemenu_callback(int action, const struct menu_item_ex *this_item,
  ...
  if (action == ACTION_REQUEST_MENUITEM && this_item == &file_menu &&
...
MENUITEM_SETTING(usb_charging, &global_settings.usb_charging,
...
#if defined(DX50) || defined(DX90) ||                                          \
    (defined(HAVE_USB_POWER) && !defined(USB_NONE) && !defined(SIMULATOR))
...
MENUITEM_SETTING(max_files_in_playlist, &global_settings.max_files_in_playlist,
                 NULL);
...
MAKE_MENU(limits_menu, ID2P(LANG_LIMITS_MENU), 0, Icon_NOICON,
          &max_files_in_dir, &max_files_in_playlist, &default_glyphs);
...
MENUITEM_SETTING(volume_adjust_norm_steps,
                 &global_settings.volume_adjust_norm_steps, NULL);
#endif
...
MAKE_MENU(keyclick_menu, ID2P(LANG_KEYCLICK), 0, Icon_NOICON, &keyclick,
          &keyclick_hardware, &keyclick_repeats);
#else
MAKE_MENU(keyclick_menu, ID2P(LANG_KEYCLICK), 0, Icon_NOICON, &keyclick,
          &keyclick_repeats);
#endif
...
MENUITEM_SETTING(car_adapter_mode_delay,
                 &global_settings.car_adapter_mode_delay, NULL);
...
MENUITEM_SETTING(usb_skip_first_drive, &global_settings.usb_skip_first_drive,
                 NULL);
#endif
...
MENUITEM_SETTING(buttonlight_timeout, &global_settings.buttonlight_timeout,
                 NULL);
#endif

#ifdef HAVE_BUTTONLIGHT_BRIGHTNESS
MENUITEM_SETTING(buttonlight_brightness,
                 &global_settings.buttonlight_brightness, NULL);
#endif

#ifdef HAVE_TOUCHPAD_SENSITIVITY_SETTING
MENUITEM_SETTING(touchpad_sensitivity, &global_settings.touchpad_sensitivity,
                 NULL);
#endif
...
MAKE_MENU(sel_softlock, ID2P(LANG_SOFTLOCK_SELECTIVE), NULL, Icon_Menu_setting,
...
static int ram_info_action_callback(int action, struct gui_synclist *lists) {
  (void)lists;
  if (action == ACTION_REDRAW || action == ACTION_NONE) {
    extern struct buflib_context core_ctx;

    simplelist_reset_lines();

    size_t total = (unsigned char *)core_ctx.alloc_end -
                   (unsigned char *)core_ctx.buf_start;
    size_t free = core_available();
    size_t used = total - free;

    simplelist_addline("Total RAM: %lu KB", (unsigned long)total / 1024);
    simplelist_addline("Used RAM:  %lu KB", (unsigned long)used / 1024);
    simplelist_addline("Free RAM:  %lu KB", (unsigned long)free / 1024);
    simplelist_addline("Max Block: %lu KB",
                       (unsigned long)core_allocatable() / 1024);
  }
  return action;
}

static int dbg_ram_usage(void) {
  struct simplelist_info info;

  simplelist_info_init(&info, ID2P(LANG_RAM_INFO), 0, NULL);
  simplelist_setline(
      "Loading..."); /* set a line so list is not empty on load */
  info.action_callback = ram_info_action_callback;

  return simplelist_show_list(&info) ? 1 : 0;
}

MENUITEM_FUNCTION(ram_info_item, 0, ID2P(LANG_RAM_INFO), dbg_ram_usage, NULL,
                  Icon_System_menu);

MAKE_MENU(system_menu, ID2P(LANG_SYSTEM), 0, Icon_System_menu,
          ...
          &limits_menu,
#ifdef HAVE_PERCEPTUAL_VOLUME
          &volume_adjust_mode, &volume_adjust_norm_steps,
#endif
#ifdef HAVE_QUICKSCREEN
...
char *sleeptimer_getname(int selected_item, void *data, char *buffer,
                         size_t buffer_len) {
  (void)selected_item;
  (void)data;
...
MENUITEM_FUNCTION_DYNTEXT(sleeptimer_toggle, 0, toggle_sleeptimer,
                          sleeptimer_getname, sleeptimer_voice, NULL, NULL,
                          Icon_NOICON);
MENUITEM_SETTING(sleeptimer_duration, &global_settings.sleeptimer_duration,
                 sleeptimer_duration_cb);
MENUITEM_SETTING(sleeptimer_on_startup, &global_settings.sleeptimer_on_startup,
                 NULL);
...
MAKE_MENU(startup_shutdown_menu, ID2P(LANG_STARTUP_SHUTDOWN), 0,
          Icon_System_menu, &show_shutdown_message, &start_screen, &poweroff,
          &sleeptimer_toggle, &sleeptimer_duration, &sleeptimer_on_startup,
...
MENUITEM_SETTING(autocreatebookmark, &global_settings.autocreatebookmark,
                 bmark_callback);
...
MAKE_MENU(bookmark_settings_menu, ID2P(LANG_BOOKMARK_SETTINGS), 0,
          Icon_Bookmark, &autocreatebookmark, &autoupdatebookmark,
          &autoloadbookmark, &usemrb);
/*    BOOKMARK MENU                */
/***********************************/
...
static int autoresume_callback(int action, const struct menu_item_ex *this_item,
  ...
  if (action == ACTION_EXIT_MENUITEM /* on exit */
      && global_settings.autoresume_enable && !tagcache_is_usable()) {
    static const char *lines[] = {ID2P(LANG_TAGCACHE_BUSY),
                                  ID2P(LANG_TAGCACHE_FORCE_UPDATE)};
    static const struct text_message message = {lines, 2};
...
MAKE_MENU(autoresume_menu, ID2P(LANG_AUTORESUME), 0, Icon_NOICON,
...
MENUITEM_SETTING(talk_dir_clip_item, &global_settings.talk_dir_clip,
                 talk_callback);
MENUITEM_SETTING(talk_file_item, &global_settings.talk_file, NULL);
MENUITEM_SETTING(talk_file_clip_item, &global_settings.talk_file_clip,
                 talk_callback);
static int talk_callback(int action, const struct menu_item_ex *this_item,
                         struct gui_synclist *this_list) {
  (void)this_list;
...
MENUITEM_SETTING(talk_battery_level_item, &global_settings.talk_battery_level,
                 NULL);
MENUITEM_SETTING(talk_mixer_amp_item, &global_settings.talk_mixer_amp, NULL);
MAKE_MENU(voice_settings_menu, ID2P(LANG_VOICE), 0, Icon_Voice, &talk_menu_item,
          &talk_dir_item, &talk_dir_clip_item, &talk_file_item,
          &talk_file_clip_item, &talk_filetype_item, &talk_battery_level_item,
          &talk_mixer_amp_item);
/*    VOICE MENU                   */
/***********************************/
...
MENUITEM_FUNCTION(wps_set_context_plugin, 0,
                  ID2P(LANG_OPEN_PLUGIN_SET_WPS_CONTEXT_PLUGIN), wps_plugin_cb,
                  NULL, Icon_Plugin);
...
MENUITEM_SETTING(browser_default, &global_settings.browser_default, NULL);

#ifdef HAVE_HOTKEY
MENUITEM_SETTING(hotkey_wps_item, &global_settings.hotkey_wps, NULL);
#endif

MAKE_MENU(wps_settings, ID2P(LANG_WPS), 0, Icon_Playback_menu, &browser_default
#ifdef HAVE_HOTKEY
          ,
          &hotkey_wps_item
#endif
...
MENUITEM_FUNCTION_W_PARAM(browse_langs, 0, ID2P(LANG_LANGUAGE), browse_folder,
                          (void *)&langs, NULL, Icon_Language);

MAKE_MENU(settings_menu_item, ID2P(LANG_GENERAL_SETTINGS), 0,
          Icon_General_settings_menu, &wps_settings, &playlist_settings,
          &file_menu,
#ifdef HAVE_TAGCACHE
          &tagcache_menu,
#endif
          &display_menu, &system_menu, &startup_shutdown_menu,
