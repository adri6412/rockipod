...
#include "filetypes.h"
#include "font.h"
#include "gui/transitions.h" /* For slide animations */
#include "icons.h"
#include "kernel.h"
#include "keyboard.h"
#include "lang.h"
...
static bool trigger_transition = false; /* Trigger for slide animation */
...
static int tree_voice_cb(int selected_item, void *data) {
  ...
  if (!did_clip) {
    ...
    case 1: /* as numbers */
      ...
      talk_number(selected_item + 1 - (is_dir ? 0 : local_tc->dirsindir), true);
      break;
    case 2: /* spelled */
      talk_shutup();
...
bool check_rockboxdir(void) {
  if (!dir_exists(ROCKBOX_DIR)) { /* No need to localise this message.
                                     If .rockbox is missing, it wouldn't work
                                     anyway */
    FOR_NB_SCREENS(i)
...
struct tree_context *tree_get_context(void) { return &tc; }
...
#if ((CONFIG_PLATFORM & PLATFORM_NATIVE) || defined(__APPLE__) ||              \
...
static int update_dir(void) {
  ...
  /* Checks for changes */
  if (id3db) {
    if (tc.currtable != lasttable || tc.currextra != lastextra || reload_dir) {
  ...
  gui_synclist_set_icon_callback(
      list, global_settings.show_icons ? tree_get_fileicon : NULL);
  ...
  if (trigger_transition) {
    /* Slide animation when changing directories! */
    transition_start(TRANSITION_FADE, &screens[SCREEN_MAIN], 200);
    trigger_transition = false;

    /* Redraw again? No, transition_start updates screen.
       But loop might redraw again? No update_dir is called once per dir
       change.
    */
  }
...
void reload_directory(void) { reload_dir = true; }
...
void set_dirfilter(int l_dirfilter) { *tc.dirfilter = l_dirfilter; }
...
void set_current_file(const char *path) { set_current_file_ex(path, NULL); }
...
static int dirbrowse(void) {
  ...
  if (*tc.dirfilter > NUM_FILTER_MODES && numentries == 0) {
    splash(HZ * 2, *tc.dirfilter == SHOW_M3U ? ID2P(LANG_CATALOG_NO_PLAYLISTS)
                                             : ID2P(LANG_NO_FILES));
    return exit_to_new_screen(
        GO_TO_PREVIOUS); /* No files found for rockbox_browse() */
  }
  ...
  while (tc.browse && tc.is_browsing) {
    ...
    if (id3db && (button == ACTION_STD_OK || button == ACTION_STD_CONTEXT)) {
      ...
      if (customaction == ONPLAY_CUSTOMACTION_SHUFFLE_SONGS) {
        /* The code to insert shuffled is on the context branch of the switch
         * so we always go here */
    ...
    case ACTION_STD_OK:
...

      {
        ...
        if (entry && (entry->attr & ATTR_DIRECTORY)) {
          /* Capture screen before entering directory */
          transition_capture_screen(&screens[SCREEN_MAIN]);
          screens[SCREEN_MAIN].clear_display(); /* Prevent ghosting */
    ...
    case ACTION_STD_CANCEL:
      ...
      /* Capture screen before going back up a level */
      transition_capture_screen(&screens[SCREEN_MAIN]);
      screens[SCREEN_MAIN].clear_display(); /* Prevent ghosting */
      trigger_transition = true;
          ...
          if (tagtree_get_attr(&tc) == FILE_ATTR_AUDIO) {
            ...
            */
            if (tc.selected_item < tc.special_entry_count) {
              title_len =
                  snprintf(buf, sizeof(buf), "%s ", tagtree_get_title(&tc));
              if (title_len < 0)
                title_len = 0;
            }

            if (title_len < (int)sizeof(buf))
              tagtree_get_entry_name(&tc, tc.selected_item, buf + title_len,
  ...
  check_rescan:
    ...
    if (reload_dir || reload_root || lastfilter != *tc.dirfilter ||
    ...
    if (restore || reload_dir) {
      ...
      numentries = update_dir();
      reload_dir = false;
      if (currdir[1] && (numentries < 0)) { /* not in root and reload failed */
...
int create_playlist(void) {
  ...
  trigger_cpu_boost();
  ret =
      catalog_add_to_a_playlist(PATH_ROOTSTR, ATTR_DIRECTORY, true, NULL, NULL);
  cancel_cpu_boost();
...
#define FIX_PTR(x)                                                             \
  {                                                                            \
    if ((void *)x >= current &&                                                \
        (void *)x < (current + cache->name_buffer_size))                       \
      x += diff;                                                               \
  }

  if (handle == cache->name_buffer_handle) { /* update entry structs, *even if
                                                they are struct tagentry */
...
bool bookmark_play(char *resume_file, int index, unsigned long elapsed,
  ...
  if (suffix != NULL &&
      !strncasecmp(suffix, ".m3u", sizeof(".m3u") - 1)) /* gets m3u8 too */
  {
    /* Playlist playback */
    char *slash;
    ...
    if (playlist_create(resume_file, NULL) != -1) {
      ...
      for (i = 0; i < amt; i++) {
        ...
        peek_filename =
            playlist_peek(modidx, filename_buf, sizeof(filename_buf));
...
static int ft_play_dirname(char *name) {
  ...
  return talk_file(
      tc.currdir, name, dir_thumbnail_name, NULL,
      global_settings.talk_filetype ? TALK_IDARRAY(VOICE_DIR) : NULL, false);
}
...
static int ft_play_filename(char *dir, char *file, int attr) {
  ...
  return talk_file(dir, NULL, file, NULL, TALK_IDARRAY(LANG_VOICE_DIR_HOVER),
                   false);
}
...
void tree_flush(void) {
  tc.is_browsing =
      false; /* clear browse to prevent reentry to a possibly missing file */
#ifdef HAVE_TAGCACHE
  tagcache_shutdown();
#endif
