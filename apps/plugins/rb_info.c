...
#define CACHE_SIZE (0)
#endif
    {"thread stack", DEFAULT_STACK_SIZE}, {"plugin buffer", PLUGIN_BUFFER_SIZE},
    {"frame_buffer", FRAMEBUFFER_SIZE},   {"codec_buffer", CODEC_SIZE},
    {"logf_buffer", MAX_LOGF_SIZE},       {"cache", CACHE_SIZE},
};
...
#define MENU_ITEM(ID, NAME, COUNT) [ID] { NAME, MENU_ID(ID), (int)COUNT }
    MENU_ITEM(M_ROOT, "RockIpod Info Plugin", M_LAST_ITEM),
...
static void cleanup(void *parameter) { (void)parameter; }
...
static const char *menu_plugin_name_cb(int selected_item, void *data, char *buf,
                                       size_t buf_len) {
  (void)data;
  buf[0] = '\0';
  switch (selected_item) {
  case 0:
    rb->snprintf(buf, buf_len, "%s: [%d bytes] ", "plugin_api",
                 (int)sizeof(struct plugin_api));
    break;
  case 1:
    rb->snprintf(buf, buf_len, "%s: [%d bytes] ", "plugin buffer",
                 PLUGIN_BUFFER_SIZE);
    break;
  case 2:
    rb->snprintf(buf, buf_len, "%s: [%d bytes] ", "frame_buffer",
                 (int)FRAMEBUFFER_SIZE);
    break;
  case 3:
    rb->snprintf(buf, buf_len, "%s: [W: %d H:%d] ", "LCD", LCD_WIDTH,
                 LCD_HEIGHT);
    break;
  case 4:
    rb->snprintf(buf, buf_len, "%s: [%d bits] ", "fb_data",
                 (int)(sizeof(fb_data) * CHAR_BIT));
    break;
  case 5:
    break;
  }
...
static const char *list_get_name_cb(int selected_item, void *data, char *buf,
                                    size_t buf_len) {
  buf[0] = '\0';
  ...
  if (data == MENU_ID(M_PATHS)) {
    ...
    rb->snprintf(buf, buf_len, "%s: [0x%X] ", btn->name,
                 (unsigned int)btn->value);
    return buf;
    ...
    rb->snprintf(buf, buf_len, "%s: [%ld bytes] ", bufm->name,
                 (long)bufm->size);
    return buf;
    ...
    rb->snprintf(
        buf, buf_len,
        "put_line item: [ %d ]$Text %d$Text LONGER TEST text %d $4$5$6$7$8$9",
        selected_item, 1, 2);
    return buf;
  }
  return buf;
...
static int list_voice_cb(int list_index, void *data) {
  ...
  if (data == MENU_ID(M_ROOT)) {
    ...
    char buf[64];
    const char *name = printcell_get_column_text(
        printcell_get_column_selected(), buf, sizeof(buf));
    long id = P2ID((const unsigned char *)name);
    if (id >= 0)
      rb->talk_id(id, true);
    else
...
int menu_action_cb(int *action, int selected_item, bool *exit,
                   struct gui_synclist *lists) {
  if (lists->data == MENU_ID(M_TESTPUT) &&
      (selected_item < (mainitem(M_TESTPUT)->items) - 1) /*back*/) {
  ...
  if (*action == ACTION_STD_OK) {
    ...
    } else if (selected_item >= (mainitem(main_last_sel)->items) - 1) /*back*/
    {
      *action = ACTION_STD_CANCEL;
      ...
      char buf[MAX_PATH];
      const char *name =
          list_get_name_cb(selected_item, lists->data, buf, sizeof(buf));
      /* splash long enough to get fingers off button then wait for new button
       * press */
      rb->splashf(HZ / 2, "%s %d (0x%X)", name, selected_item - 1,
                  selected_item - 1);
      rb->button_get(true);
    }
  }
...
static void synclist_set(char *menu_id, int selected_item, int items,
                         int sel_size) {
  if (items <= 0)
    return;
  if (selected_item < 0)
  ...
  rb->gui_synclist_init(&lists, list_get_name_cb, menu_id, false, sel_size,
                        NULL);
  if (menu_id == MENU_ID(M_TESTPUT)) {
    testput_cols =
        printcell_set_columns(&lists, NULL, TESTPUT_HEADER, Icon_Rockbox);
  } else {
