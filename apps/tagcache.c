...
/* Tag Cache Header version 'TCHxx'. Increment when changing internal
 * structures. */
...
#define sim_sleep(timeout)                                                     \
  do {                                                                         \
  } while (0)
#define do_timed_yield()                                                       \
  do {                                                                         \
  } while (0)
...
static long tagcache_stack[(DEFAULT_STACK_SIZE + 0x20000) / sizeof(long)];
...
#define TAGCACHE_SORTED_TAGS                                                   \
  ((1LU << tag_artist) | (1LU << tag_album) | (1LU << tag_genre) |             \
   (1LU << tag_composer) | (1LU << tag_comment) | (1LU << tag_albumartist) |   \
   (1LU << tag_grouping) | (1LU << tag_title) |                                \
   (1LU << tag_virt_canonicalartist))

/* Uniqued tags (we can use these tags with filters and conditional clauses). */
#define TAGCACHE_UNIQUE_TAGS                                                   \
  ((1LU << tag_artist) | (1LU << tag_album) | (1LU << tag_genre) |             \
   (1LU << tag_composer) | (1LU << tag_comment) | (1LU << tag_albumartist) |   \
   (1LU << tag_grouping) | (1LU << tag_virt_canonicalartist))

/* String presentation of the tags defined in tagcache.h. Must be in correct
 * order! */
static const char *const tags_str[] = {
    "artist",     "album",       "genre",           "title",      "filename",
    "composer",   "comment",     "albumartist",     "grouping",   "year",
    "discnumber", "tracknumber", "canonicalartist", "bitrate",    "length",
    "playcount",  "rating",      "playtime",        "lastplayed", "commitid",
    "mtime",      "lastelapsed", "lastoffset"
...
static const char *const tag_type_str[] = {
    [clause_none] = "clause_none",
    [clause_is] = "clause_is",
    [clause_is_not] = "clause_is_not",
    [clause_gt] = "clause_gt",
...
#ifdef __PCTOOL__
#define tc_debug_log(x) printf("%s\n", x)
#else
/* static void tc_debug_log(const char *msg); */
#endif

#ifdef PLUGIN
#define tc_debug_log(x) ((void)0)
#endif
...
#ifndef __PCTOOL__
static struct tagcache_command_entry
    command_queue[TAGCACHE_COMMAND_QUEUE_LENGTH];
...
struct tagfile_entry {
  ...
  int32_t idx_id;   /* Corresponding entry location in index file of not unique
                       tags */
...
static inline void tcrc_buffer_lock(void) { core_pin(tcramcache.handle); }

static inline void tcrc_buffer_unlock(void) { core_unpin(tcramcache.handle); }
...
static inline void str_setlen(char *buf, size_t len) { buf[len] = '\0'; }

const char *tagcache_tag_to_str(int tag) { return tags_str[tag]; }
...
static enum e_read_errors read_tagfile_entry_and_tag(int fd,
...
static ssize_t read_index_entries(int fd, struct index_entry *buf,
                                  size_t count) {
...
static ssize_t write_index_entries(int fd, struct index_entry *buf,
                                   size_t count) {
#ifdef TAGCACHE_SUPPORT_FOREIGN_ENDIAN
  ssize_t ret = 0;
  for (; count > 0; count--) {
...
static int NO_INLINE open_db_fd(const char *filename, int mode) {
  ...
  /* Debug logging for path and existence */
  /*
  {
      char logbuf[128];
      snprintf(logbuf, sizeof(logbuf), "open_db_fd: opening %s (path=%s)", buf,
  tc_stat.db_path); tc_debug_log(logbuf);
  }
  */
  ...
  if (fd < 0) {
    char logbuf[128];
    snprintf(logbuf, sizeof(logbuf), "open_db_fd failed: %s (mode %x) errno=%d",
             buf, mode, errno);
    tc_debug_log(logbuf);
  }
  return fd;
...
static int open_tag_fd(struct tagcache_header *hdr, int tag, bool write) {
  ...
  fd = open_pathfmt(fname, sizeof(fname), write ? O_RDWR : O_RDONLY,
                    "%s/" TAGCACHE_FILE_INDEX, tc_stat.db_path, tag);
  if (fd < 0) {
    logf("%s failed: tag=%d write=%d file= " TAGCACHE_FILE_INDEX, __func__, tag,
         write, tag);
    char logbuf[128];
    snprintf(logbuf, sizeof(logbuf),
             "open_tag_fd failed: tag=%d write=%d errno=%d", tag, write, errno);
    tc_debug_log(logbuf);
    tc_stat.ready = false;
    return fd;
  }
...
static int open_master_fd(struct master_header *hdr, bool write) {
  ...
  if (fd < 0) {
    ...
    tc_debug_log("open_master_fd: failed to open master file");
    tc_stat.ready = false;
    return fd;
  }
  ...
  if (rc != sizeof(struct master_header)) {
    ...
    tc_debug_log("open_master_fd: read failed");
    close(fd);
    return -1;
  }
...
static void remove_files(void) {
  ...
  for (i = 0; i < TAG_COUNT; i++) {
    ...
    snprintf(buf, bufsz, "%s/" TAGCACHE_FILE_INDEX, tc_stat.db_path, i);
    remove(buf);
  }
}

static bool check_all_headers(void) {
  tc_debug_log("check_all_headers: start");
  ...
  if ((fd = open_master_fd(&myhdr, false)) < 0) {
    tc_debug_log("check_all_headers: open_master_fd failed");
    return false;
  }
  ...
  if (myhdr.dirty) {
    ...
    tc_debug_log("tagcache is dirty!");
    return false;
  }
  ...
  for (tag = 0; tag < TAG_COUNT; tag++) {
    ...
    if ((fd = open_tag_fd(&tch, tag, false)) < 0) {
      char buf[64];
      snprintf(buf, sizeof(buf), "check_all_headers: open_tag_fd failed for %d",
               tag);
      tc_debug_log(buf);
      return false;
    }
  ...
  tc_debug_log("check_all_headers: done");
  return true;
}
...
static long find_entry_ram(const char *filename) {
  ...
  if (!tc_stat.ramcache || global_settings.tagcache_ram != TAGCACHE_RAM_ON)
    return -1;
...
static bool get_index(int masterfd, int idxid, struct index_entry *idx,
  ...
  lseek(masterfd,
        idxid * sizeof(struct index_entry) + sizeof(struct master_header),
        SEEK_SET);
...
static bool open_files(struct tagcache_search *tcs, int tag) {
  if (tcs->idxfd[tag] < 0) {
    char fname[MAX_PATH];
    tcs->idxfd[tag] =
        open_pathfmt(fname, sizeof(fname), O_RDONLY, "%s/" TAGCACHE_FILE_INDEX,
                     tc_stat.db_path, tag);
    if (tcs->idxfd[tag] < 0) {
      logf("File not open!");
      return false;
...
static bool retrieve(struct tagcache_search *tcs,
                     IF_DIRCACHE(int idx_id, ) struct index_entry *idx, int tag,
                     char *buf, long bufsz) {
  bool success = false;
...
static long tc_find_tag(int tag, int idx_id, const struct index_entry *idx) {
  ...
  if (!COMMAND_QUEUE_IS_EMPTY && TAGCACHE_IS_NUMERIC(tag)) {
    ...
    if (result >= 0) {
      logf("tc_find_tag: Recovered tag %d value %lX from write queue", tag,
...
static inline long sec_in_ms(long ms) { return (ms / 1000) % 60; }

static inline long min_in_ms(long ms) { return (ms / 1000) / 60; }
...
static long check_virtual_tags(int tag, int idx_id,
  ...
  case tag_virt_entryage:
    data = current_tcmh.commitid - tc_find_tag(tag_commitid, idx_id, idx) - 1;
    break;
...
inline static bool str_begins_ends_oneof(const char *str, const char *list,
                                         bool begins) {
  logf_clauses("%s %s (%s) %s", str, __func__, begins ? "begins" : "ends",
               list);
  const char *sep;
...
static bool check_clauses(struct tagcache_search *tcs, struct index_entry *idx,
  ...
  for (i = 0; i < count; i++) {
    ...
    logf_clauses(
        "%s clause %d %s %s [%ld] %s", "Checking", i,
        tag_type_str[clause->type], tags_str[clause->tag], clause->numeric_data,
    ...
    if (tcs->ramsearch) {
      ...
      if (!TAGCACHE_IS_NUMERIC(clause->tag)) {
        if (clause->tag == tag_filename || clause->tag == tag_virt_basename) {
          retrieve(tcs, IF_DIRCACHE(tcs->idx_id, ) idx, clause->tag, buf,
                   bufsz);
        } else {
          tfe =
              (struct tagfile_entry *)&tcramcache.hdr->tags[clause->tag][seek];
...
#endif /* HAVE_TC_RAMCACHE */
    {
      ...
      if (!TAGCACHE_IS_NUMERIC(clause->tag)) {
        ...
        if (fd < 0) {
          char logbuf[64];
          snprintf(logbuf, sizeof(logbuf),
                   "Invalid FD for tag %d in check_clauses", tag);
          tc_debug_log(logbuf);
          return false;
        }
        ...
        /* tc_debug_log("check_clauses: calling read_tagfile_entry_and_tag"); */
    ...
    logf_clauses(
        "%s clause %d %s %s [%ld] %s", "Found", i, tag_type_str[clause->type],
...
static bool add_uniqbuf(struct tagcache_search *tcs, uint32_t id) {
  ...
  if (tcs->unique_list == NULL ||
      (!TAGCACHE_IS_UNIQUE(tcs->type) && !TAGCACHE_IS_NUMERIC(tcs->type))) {
    return true;
  }
...
static bool build_lookup_list(struct tagcache_search *tcs) {
  ...
  if (tcs->masterfd < 0) {
    ...
    if (tcs->masterfd < 0) {
      tc_debug_log("build_lookup_list: open_master_fd failed");
      return false;
    }
  }

  if (lseek(tcs->masterfd,
            tcs->seek_pos * sizeof(struct index_entry) +
                sizeof(struct master_header),
            SEEK_SET) < 0) {
    tc_debug_log("build_lookup_list: lseek failed");
    return false;
  }

  while (read_index_entries(tcs->masterfd, &entry, 1) ==
...
bool tagcache_search(struct tagcache_search *tcs, int tag) {
  /* NOTE: call tagcache_search_finish(&tcs) when finished or BAD things may
   * happen (TM) */
  ...
  if (tc_stat.commit_step > 0 || !tc_stat.ready) {
    tc_debug_log("tagcache_search: not ready");
    return false;
  }
  ...
  {
    char logbuf[64];
    snprintf(logbuf, sizeof(logbuf), "tagcache_search: start tag=%d", tag);
    tc_debug_log(logbuf);
  }
...
#endif
  {
     ...
     * without failing. */
    /* But for M3K, let's try RO first to avoid locking issues */
    tcs->masterfd = open_master_fd(&master_hdr, false);
    if (tcs->masterfd < 0) {
      tc_debug_log("tagcache_search: open_master_fd failed");
      return false;
    }
  ...
  tc_debug_log("tagcache_search: success");

  return true;
}

void tagcache_search_set_uniqbuf(struct tagcache_search *tcs, void *buffer,
                                 long length) {
...
bool tagcache_search_add_filter(struct tagcache_search *tcs, int tag,
                                int seek) {
...
bool tagcache_search_add_clause(struct tagcache_search *tcs,
  ...
  if (clause->type != clause_logical_or) {
    ...
    if (clause_count <= 1 ||
        tcs->clause[clause_count - 1]->type != clause_logical_or) {
    ...
    if (!TAGCACHE_IS_NUMERIC(clause->tag) && tcs->idxfd[clause->tag] < 0) {
      char fname[MAX_PATH];
      tcs->idxfd[clause->tag] =
          open_pathfmt(fname, sizeof(fname), O_RDONLY,
                       "%s/" TAGCACHE_FILE_INDEX, tc_stat.db_path, clause->tag);
    }
  }
...
static bool get_next(struct tagcache_search *tcs, bool is_numeric, char *buf,
                     long bufsz) {
  ...
  if (tcs->idxfd[tcs->type] < 0 && !is_numeric
    ...
    tc_debug_log("get_next: invalid fd");
    return false;
  }
  ...
  if (tcs->ramsearch) {
    ...
    if (tcs->type == tag_filename && (flag & FLAG_DIRCACHE)) {
      ssize_t len =
          dircache_get_fileref_path(&tcrc_dcfrefs[tcs->idx_id], buf, bufsz);
      if (len >= 0) {
    ...
    if (tcs->type != tag_filename) {
      ...
      ep = (struct tagfile_entry *)&tcramcache.hdr
               ->tags[tcs->type][tcs->position];
  ...
  switch (
      read_tagfile_entry_and_tag(tcs->idxfd[tcs->type], &entry, buf, bufsz)) {
  case e_SUCCESS_LEN_ZERO:
  case e_SUCCESS:
    break;
  case e_ENTRY_SIZEMISMATCH:
...
bool tagcache_get_next(struct tagcache_search *tcs, char *buf, long size) {
  ...
  if (tcs->unique_list_count > 0)
    logf(" uniqbuf: %d used / %d avail", tcs->unique_list_count,
         tcs->unique_list_capacity);
#endif

  return false;
}

bool tagcache_retrieve(struct tagcache_search *tcs, int idxid, int tag,
                       char *buf, long size) {
  struct index_entry idx;
...
static struct tagfile_entry *get_tag(const struct index_entry *entry, int tag) {
  return (struct tagfile_entry *)&tcramcache.hdr
      ->tags[tag][entry->tag_seek[tag]];
}
...
static void db_log(const char *prefix, const char *msg) {
  ...
  int logfd =
      open(ROCKBOX_DIR "/database.log", O_WRONLY | O_APPEND | O_CREAT, 0666);
  if (logfd >= 0) {
...
#define ADD_TAG(entry, tag, data)                                              \
  ...
  /* String tags. */
  has_artist = id3.artist != NULL && strlen(id3.artist) > 0;
  has_grouping = id3.grouping != NULL && strlen(id3.grouping) > 0;
...
static bool tempbuf_insert(char *str, int id, int idx_id, bool unique) {
  struct tempbuf_searchidx *index =
      (struct tempbuf_searchidx *)((uintptr_t)tempbuf & ~0x3);
  ...
  if (tempbuf_left - 4 < 0 || tempbufidx >= commit_entry_count) {
    logf("temp buf error rem: %ld idx: %ld / %ld", tempbuf_left, tempbufidx,
...
static int compare(const void *p1, const void *p2) {
  /* Removing do_timed_yield to prevent stack usage/interrupts during qsort */
  /* do_timed_yield(); */
...
static int tempbuf_sort(int fd) {
  ...
  /* Verify index pointer alignment */
  if (((uintptr_t)index & 0x3) != 0) {
    logf("FATAL: index misaligned %p", index);
    return -1;
  }

  /* Verify qsort params */
  if (tempbufidx > 0 && index == NULL) {
    logf("FATAL: qsort NULL base");
    tc_debug_log("FATAL: qsort NULL base");
    return -1;
  }

  logf("qsort start: %ld items", tempbufidx);
  {
    char buf[64];
    snprintf(buf, sizeof(buf), "qsort start: %ld items", tempbufidx);
    tc_debug_log(buf);
  }
  qsort(index, tempbufidx, sizeof(struct tempbuf_searchidx), compare);
  logf("qsort done");
  tc_debug_log("qsort done");
  ...
  /* qsort was moved up to detect crashes */
  /* qsort(index, tempbufidx, sizeof(struct tempbuf_searchidx), compare); */
...
static bool build_numeric_indices(struct tagcache_header *h, int tmpfd) {
  ...
  int masterfd;
  int masterfd_pos;
  struct temp_file_entry *entrybuf =
      (struct temp_file_entry *)((uintptr_t)tempbuf & ~0x3);
  int max_entries;
  ...
  masterfd_pos = lseek(
      masterfd, tcmh.tch.entry_count * sizeof(struct index_entry), SEEK_CUR);
  if (masterfd_pos < 0) {
...
#define tmpdb_read_string_tag(tag)                                             \
  ...
  str_setlen(build_idx_buf, tfe->tag_length[tag]);                             \
                                                                               \
  tfe->tag_offset[tag] =                                                       \
      crc_32(build_idx_buf, strlen(build_idx_buf), 0xffffffff);                \
...
#define tmpdb_copy_tag(tag) tfe->tag_offset[tag] = idx.tag_seek[tag]
        ...
        if (write_index_entries(masterfd, &idx, 1) !=
            sizeof(struct index_entry)) {
...
static int build_index(int index_type, struct tagcache_header *h, int tmpfd) {
  ...
  if (fd < 0) {
    /* tc_debug_log("build_index: open_tag_fd failed, trying create"); */
  }
  if (fd >= 0) {
  ...
  /* ALIGNMENT FIX: Ensure tempbuf_pos is aligned to sizeof(void*) */
  tempbuf_pos = (tempbuf_pos + sizeof(void *) - 1) & ~(sizeof(void *) - 1);
  ...
  if (fd >= 0) {
    ...
    if (TAGCACHE_IS_SORTED(index_type)) {
      ...
      for (i = 0; i < tch.entry_count && !USR_CANCEL; i++) {
        ...
        bool ret;
        switch (read_tagfile_entry_and_tag(fd, &entry, build_idx_buf,
                                           build_idx_bufsz)) {
         ...
         */
        ret = tempbuf_insert(build_idx_buf,
                             loc / TAGFILE_ENTRY_CHUNK_LENGTH +
                                 commit_entry_count,
                             entry.idx_id, TAGCACHE_IS_UNIQUE(index_type));
        if (!ret) {
          close(fd);
          return -3;
        }
    ...
    fd = open_pathfmt(build_idx_buf, build_idx_bufsz,
                      O_WRONLY | O_CREAT | O_TRUNC, "%s/" TAGCACHE_FILE_INDEX,
  ...
  if (masterfd < 0) {
    tc_debug_log("build_index: failed to open masterfd R/W, trying create");
    ...
    if (masterfd < 0) {
      ...
      tc_debug_log("build_index: failed to create masterfd");
      close(fd);
      return -2;
    }
    ...
    for (i = 0; i < h->entry_count && !USR_CANCEL; i++) {
...
#endif /*defined(PLUGIN)*/
      {
        if (TAGCACHE_IS_UNIQUE(index_type))
          ...
          error = !tempbuf_insert(build_idx_buf, i, tcmh.tch.entry_count + i,
                                  false);
      ...
      /* Skip to next. */
      lseek(tmpfd,
            entry.data_length - entry.tag_offset[index_type] -
                entry.tag_length[index_type],
            SEEK_CUR);
      do_timed_yield();
    }
    ...
    for (i = 0; i < tcmh.tch.entry_count && !USR_CANCEL; i += idxbuf_pos) {
      ...
      for (j = 0; j < idxbuf_pos; j++) {
        ...
        if (idxbuf[j].tag_seek[index_type] < 0) {
          logf("update error: %" PRId32 "/%d/%" PRId32, idxbuf[j].flag, i + j,
               tcmh.tch.entry_count);
          error = true;
          goto error_exit;
        }
...
static bool commit(void) {
  ...
  if (tempbuf_size == 0 && tc_stat.ramcache_allocated > 0) {
    ...
    tempbuf_size =
        tc_stat.ramcache_allocated - sizeof(struct ramcache_header) - 128;
    tempbuf_size &= ~0x03;
  ...
  for (i = 0; i < TAG_COUNT && !USR_CANCEL; i++) {
    ...
    {
      char buf[64];
      snprintf(buf, sizeof(buf), "build_index start: %d", i);
      tc_debug_log(buf);
    }
    ...
    {
      char buf[64];
      snprintf(buf, sizeof(buf), "build_index done: %d (ret=%d)", i, ret);
      tc_debug_log(buf);
    }
  ...
  tc_debug_log("build_numeric_indices start");
  if (!build_numeric_indices(&tch, tmpfd)) {
    tc_debug_log("build_numeric_indices failed");
    logf("Failure to commit numeric indices");
    close(tmpfd);
    tc_stat.commit_step = 0;
    goto commit_error;
  }
  tc_debug_log("build_numeric_indices done");
  ...
  if (!USR_CANCEL) {
    tc_debug_log("Updating master index headers");
    /* Update the master index headers. */
    if ((masterfd = open_master_fd(&tcmh, true)) < 0) {
      tc_debug_log("open_master_fd failed");
      goto commit_error;
    }

    tc_debug_log("Removing temp file");
    remove_db_file(TAGCACHE_FILE_TEMP);
    ...
    tc_debug_log("Writing master header");
    write_master_header(masterfd, &tcmh);
    close(masterfd);

    logf("tagcache committed");
    tc_debug_log("tagcache committed");
...
void tagcache_commit_finalize(void) {
  tc_debug_log("tagcache_commit_finalize: calling check_all_headers");
  tc_stat.ready = check_all_headers();
  tc_debug_log("tagcache_commit_finalize: check_all_headers returned");
  tc_stat.readyvalid = true;
  tc_debug_log("tagcache_commit_finalize: done");
}
...
static bool read_tag(char *dest, long size, const char *src,
                     const char *tagstr) {
  int pos;
  char current_tag[32];
...
bool tagcache_create_changelog(struct tagcache_search *tcs) {
  ...
  for (i = 0; i < myhdr.tch.entry_count; i++) {
    if (read_index_entries(tcs->masterfd, &idx, 1) !=
        sizeof(struct index_entry)) {
      logf("read error #9");
      tagcache_search_finish(tcs);
      close(clfd);
...
static bool delete_entry(long idx_id) {
  ...
  for (tag = 0; tag < TAG_COUNT; tag++) {
...
#endif /* HAVE_TC_RAMCACHE */
    {
      ...
      switch (read_tagfile_entry_and_tag(fd, &tfe, build_idx_buf,
                                         build_idx_bufsz)) {
      case e_SUCCESS_LEN_ZERO:
  ...
  lseek(masterfd,
        sizeof(struct master_header) + (idx_id * sizeof(struct index_entry)),
        SEEK_SET);
  if (write_index_entries(masterfd, &myidx, 1) != sizeof(struct index_entry)) {
...
static bool load_tagcache(void) {
  tc_debug_log("load_tagcache: called");
  return false; /* DISABLE RAMCACHE FOR TESTING */
  ...
  bool ok = false;
  ssize_t bytesleft =
      tc_stat.ramcache_allocated - sizeof(struct ramcache_header);
  int fd;
  ...
  tc_debug_log("load_tagcache: opening master file");
  ...
  if (fd < 0) {
    ...
    tc_debug_log("load_tagcache: open failed");
    goto failure;
  }
  ...
  if (read_master_header(fd, &tcmh) != sizeof(struct master_header) ||
    ...
    tc_debug_log("load_tagcache: incorrect header");
    goto failure;
  }
  ...
  for (int i = 0; i < tcmh.tch.entry_count; i++) {
    ...
    if (rc != sizeof(struct index_entry)) {
      logf("read error #10");
      tc_debug_log("load_tagcache: read error #10");
      goto failure;
    }
  }
  ...
  tc_debug_log("load_tagcache: loading tags");
  ...
  for (int tag = 0; tag < TAG_COUNT; tag++) {
    ...
    /* char buf[64]; snprintf(buf, sizeof(buf), "load_tagcache: loading tag %d",
     * tag); tc_debug_log(buf); */
    fd = open_tag_fd(tch, tag, false);
    if (rc < 0) {
      tc_debug_log("load_tagcache: open_tag_fd failed");
      goto failure;
    }
    ...
    for (tcramcache.hdr->entry_count[tag] = 0;
      ...
      if (do_timed_yield() && check_event_queue()) {
        tc_debug_log("load_tagcache: aborted by queue");
        goto failure;
      }
      ...
      if (bytesleft < (ssize_t)sizeof(struct tagfile_entry)) {
        ...
        tc_debug_log("load_tagcache: too big tagcache #10.75");
        goto failure;
      }
      ...
      if (idx_id != -1 || tag == tag_filename) /* filename NOT optional */
      {
        ...
        if (idx->tag_seek[tag] != pos) {
          ...
          logf("  tag_seek[%d]=%" PRId32 ":pos=%ld", tag, idx->tag_seek[tag],
               (long)pos);
          goto failure;
        }
      }
  ...
  logf("utilization: %d%%",
       100 * tc_stat.ramcache_used / tc_stat.ramcache_allocated);
...
static bool check_deleted_files(void) { return check_file_refs(true); }

/* Note that this function must not be inlined, otherwise the whole point
 * of having the code in a separate function is lost.
 */
static void NO_INLINE check_ignore(const char *dirname, int *ignore,
                                   int *unignore) {
  /* MOVED FROM STACK TO STATIC to prevent stack overflow in deep recursion */
...
#define add_search_root(a)                                                     \
  do {                                                                         \
  } while (0)
#define free_search_roots(a)                                                   \
  do {                                                                         \
  } while (0)
#endif
...
#ifndef PLUGIN
  int fd =
      open("/.rockbox/tagcache_log.txt", O_RDWR | O_CREAT | O_APPEND, 0666);
  if (fd >= 0) {
...
static bool check_dir(const char *dirname, int add_files, int depth) {
  ...
  if (depth > 20) {
    tc_debug_log("Max depth reached");
    logf("Max depth reached: %s", dirname);
    return true;
  }
  ...
  if (!dir) {
    char b[MAX_PATH + 64];
    snprintf(b, sizeof(b), "tagcache: opendir(%s) failed", dirname);
    tc_debug_log(b);
  ...
  while (!check_event_queue()) {
    ...
    if (entry == NULL) {
      logf("DEBUG: readdir returned NULL. errno=%d", errno);
      // fflush(stdout);
      char b[MAX_PATH + 64];
      snprintf(b, sizeof(b), "tagcache: readdir(%s) returned NULL", dirname);
      tc_debug_log(b);
      success = true;
      break;
    }
    ...
    char b[MAX_PATH + 64];
    snprintf(b, sizeof(b), "Found %s", entry->d_name);
    tc_debug_log(b);
    ...
    /* Debug log */
    logf("DEBUG: Scanning %s (attr=%x)", curpath, info.attribute);
    // fflush(stdout);
...
void tagcache_screensync_event(void) { tc_stat.curentry = NULL; }

void tagcache_screensync_enable(bool state) { tc_stat.syncscreen = state; }
...
static
  ...
  /* #if !defined(__PCTOOL__) && !defined(SIMULATOR)
      return;
  #endif */
  tc_debug_log("Starting tagcache build...");
  ...
  roots_ll[0].next = NULL;
    ...
    !defined(__PCTOOL__) && !defined(APPLICATION)
  ...
  for (this = &roots_ll[0]; this; this = this->next) {
    char b[MAX_PATH + 64];
    snprintf(b, sizeof(b), "Search root %s", this->path);
    tc_debug_log(b);
    ...
    if (ret) {
      if (dir_exists(this->path))
        ...
        snprintf(b, sizeof(b), "Dir not found %s", this->path);
        tc_debug_log(b);
  ...
  if (commit()) {
    logf("tagcache built!");
    tc_debug_log("tagcache built! (commit returned true)");
  } else {
    tc_debug_log("commit returned false");
  }
#ifdef __PCTOOL__
  free_tempbuf();
#endif

  tc_debug_log("Checking HAVE_TC_RAMCACHE");
#ifdef HAVE_TC_RAMCACHE
  tc_debug_log("HAVE_TC_RAMCACHE is defined");
  if (tcramcache.hdr) {
    /* Import runtime statistics if we just initialized the db. */
    if (current_tcmh.serial == 0) {
      tc_debug_log("Posting Q_IMPORT_CHANGELOG event");
      queue_post(&tagcache_queue, Q_IMPORT_CHANGELOG, 0);
    } else {
      tc_debug_log("Serial is not 0, skipping import");
    }
  } else {
    tc_debug_log("tcramcache.hdr is NULL");
  }
#else
  tc_debug_log("HAVE_TC_RAMCACHE is NOT defined");
#endif

  tc_debug_log("Disabling cpu_boost");
  cpu_boost(false);
  tc_debug_log("do_tagcache_build finished");
}
...
static void tagcache_thread(void) {
  ...
  if (db_file_exists(TAGCACHE_FILE_TEMP)) {
    ...
    if (gui_syncyesno_run_w_tmo(HZ * 5, YESNO_YES, &message, NULL, NULL) ==
        YESNO_YES)
#endif
    {
      allocate_tempbuf();
  ...
  if (global_settings.tagcache_ram && !tc_stat.ramcache) {
    /* tc_debug_log("tagcache_thread: calling allocate_tagcache"); */
    allocate_tagcache();
  }
#endif /* HAVE_TC_RAMCACHE */
  ...
  if (!tc_stat.ready) {
    /* tc_debug_log("tagcache_thread: sleeping before commit_finalize"); */
    sleep(HZ);
    /* tc_debug_log("tagcache_thread: calling commit_finalize"); */
    tagcache_commit_finalize();
  }
  ...
  while (1) {
    ...
    case Q_IMPORT_CHANGELOG:
      tc_debug_log("tagcache_thread: Q_IMPORT_CHANGELOG");
      tagcache_import_changelog();
      break;

    case Q_REBUILD:
      tc_debug_log("tagcache_thread: Q_REBUILD");
      remove_files();
      remove_db_file(TAGCACHE_FILE_TEMP);
      tagcache_build();
      break;
    ...
    case Q_UPDATE:
      tc_debug_log("tagcache_thread: Q_UPDATE");
      tagcache_build();
#ifdef HAVE_TC_RAMCACHE
      load_ramcache();
#endif
    ...
    case Q_START_SCAN:
      tc_debug_log("tagcache_thread: Q_START_SCAN");
      check_done = false;
      /* fallthrough */
    case SYS_TIMEOUT:
      ...
      if (!tc_stat.ramcache && global_settings.tagcache_ram) {
        tc_debug_log("tagcache_thread: timeout - loading ramcache");
        load_ramcache();
        ...
        if (global_settings.tagcache_autoupdate) {
          tc_debug_log("tagcache_thread: timeout - building");
          tagcache_build();
...
void tagcache_remove_statefile(void) { remove_db_file(TAGCACHE_STATEFILE); }
...
void tagcache_start_scan(void) { queue_post(&tagcache_queue, Q_START_SCAN, 0); }
...
void tagcache_stop_scan(void) { queue_post(&tagcache_queue, Q_STOP_SCAN, 0); }
...
void tagcache_init(void) {
  ...
  if (!dir_exists(tc_stat.db_path)) {
    /* If it's just the root, it should exist. If it's .rockbox, it should too.
     */
    /* But maybe the setting is wrong or empty? */
    if (strlen(tc_stat.db_path) == 0) {
      strcpy(tc_stat.db_path, "/.rockbox");
    }

    if (!dir_exists(tc_stat.db_path)) {
      mkdir(tc_stat.db_path);
    }
  }

  mutex_init(&command_queue_mutex);
  queue_init(&tagcache_queue, true);
  create_thread(tagcache_thread, tagcache_stack, sizeof(tagcache_stack), 0,
                tagcache_thread_name IF_PRIO(, PRIORITY_BACKGROUND)
                    IF_COP(, CPU));
#else
  /* use default DB path */
  ...
  /* FILESYSTEM TEST */
  {
    int tfd = open("/.rockbox/test_fs.txt", O_WRONLY | O_CREAT | O_TRUNC, 0666);
    if (tfd >= 0) {
      write(tfd, "TEST", 4);
      close(tfd);
      tc_debug_log("Filesystem Test: Create/Write OK");

      tfd = open("/.rockbox/test_fs.txt", O_RDWR);
      if (tfd >= 0) {
        tc_debug_log("Filesystem Test: Re-open R/W OK");
        close(tfd);
      } else {
        char b[64];
        snprintf(b, sizeof(b), "Filesystem Test: Re-open R/W FAIL errno=%d",
                 errno);
        tc_debug_log(b);
      }

      tfd = open("/.rockbox/test_fs.txt", O_RDONLY);
      if (tfd >= 0) {
        tc_debug_log("Filesystem Test: Re-open RO OK");
        close(tfd);
      } else {
        char b[64];
        snprintf(b, sizeof(b), "Filesystem Test: Re-open RO FAIL errno=%d",
                 errno);
        tc_debug_log(b);
      }
      ...
      char b[64];
      snprintf(b, sizeof(b), "Filesystem Test: Create FAIL errno=%d", errno);
      tc_debug_log(b);
    }
  }
}
...
bool tagcache_is_initialized(void) { return tc_stat.initialized; }
bool tagcache_is_fully_initialized(void) { return tc_stat.readyvalid; }
bool tagcache_is_usable(void) { return tc_stat.initialized && tc_stat.ready; }
#ifdef HAVE_TC_RAMCACHE
bool tagcache_is_in_ram(void) { return tc_stat.ramcache; }
#endif
int tagcache_get_commit_step(void) { return tc_stat.commit_step; }
int tagcache_get_max_commit_step(void) { return (int)(SORTED_TAGS_COUNT) + 1; }
#endif /*!defined(PLUGIN)*/
