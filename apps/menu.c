...
static int empty_menu_callback(int action, const struct menu_item_ex *this_item,
...
static bool query_audio_status(int *old_audio_status) {
  ...
  if (*old_audio_status !=
      new_audio_status) { /* force a redraw if anything changed the audio status
                             from outside */
...
static int get_menu_selection(int selected_item,
                              const struct menu_item_ex *menu) {
...
static const char *get_menu_item_name(int selected_item, void *data,
  ...
  if (type == MT_RETURN_ID) {
    if (menu->flags & MENU_DYNAMIC_DESC) {
      name = menu->menu_get_name_and_icon->list_get_name(
          selected_item, menu->menu_get_name_and_icon->list_get_name_data,
          buffer, buffer_len);
    } else
  ...
  if ((menu->flags & MENU_DYNAMIC_DESC) && (type != MT_SETTING_W_TEXT))
    return menu->menu_get_name_and_icon->list_get_name(
        selected_item, menu->menu_get_name_and_icon->list_get_name_data, buffer,
        buffer_len);
...
static int init_menu_lists(const struct menu_item_ex *menu,
  ...
  for (i = 0; i < count; i++) {
    ...
    if (menu_callback(ACTION_REQUEST_MENUITEM,
                      type == MT_RETURN_ID ? (void *)(intptr_t)i
                                           : menu->submenus[i],
                      lists) != ACTION_EXIT_MENUITEM) {
  ...
  if (menu->flags & MENU_HAS_DESC) {
    ...
    title = menu->menu_get_name_and_icon->list_get_name(
        -1, menu->menu_get_name_and_icon->list_get_name_data, buffer,
        sizeof(buffer));
  } else {
    icon = Icon_NOICON;
    title = "";
  ...
  gui_synclist_set_icon_callback(
      lists, global_settings.show_icons ? menu_get_icon : NULL);
    ...
    if ((type == MT_SETTING) || (type == MT_SETTING_W_TEXT))
      ...
      if (menu->submenus[sel]->flags & (MENU_DYNAMIC_DESC)) {
        int (*list_speak_item)(int selected_item, void *data) =
            menu->submenus[sel]->menu_get_name_and_icon->list_speak_item;
        if (list_speak_item)
          list_speak_item(
              sel,
              menu->submenus[sel]->menu_get_name_and_icon->list_get_name_data);
        else {
          char buffer[80];
          str = menu->submenus[sel]->menu_get_name_and_icon->list_get_name(
              sel,
              menu->submenus[sel]->menu_get_name_and_icon->list_get_name_data,
...
int do_menu(const struct menu_item_ex *start_menu, int *start_selected,
  ...
  /* Capture initial screen for transitions */
    ...
    if (LIKELY(gui_synclist_do_button(&lists, &action)))
      ...
      if (menu == &root_menu_) {
        ...
        if (type == MT_SETTING_W_TEXT || type == MT_SETTING) {
          ...
          MENUITEM_STRINGLIST(settings_op_menu, ID2P(LANG_ONPLAY_MENU_TITLE),
                              NULL, ID2P(LANG_RESET_SETTING),
#ifndef HAVE_QUICKSCREEN
          );
          context_menu = &settings_op_menu;
#else
                              ID2P(LANG_TOP_QS_ITEM), ID2P(LANG_LEFT_QS_ITEM),
      ...
      if (stack_top > 0) {
        /* Capture current screen before going back */
        transition_capture_screen(&screens[SCREEN_MAIN]);
      ...
      case MT_MENU:
        if (stack_top < MAX_MENUS) {
          /* Capture screen before entering submenu */
          transition_capture_screen(&screens[SCREEN_MAIN]);
          screens[SCREEN_MAIN].clear_display(); /* Prevent ghosting */
          ...
          /* Now copy the new screen to prev_screen to blend FROM old TO new */
          /* We need to save the new screen state and restore old for blending
           */
          transition_start(TRANSITION_FADE, &screens[SCREEN_MAIN],
                           200); /* 200ms fade */

          redraw_lists = true; /* Redraw to show menu after flash */
        }
        break;
  ...
  if (start_selected) {
    ...
    *start_selected =
        get_menu_selection(gui_synclist_get_sel_pos(&lists), menu);
  }
